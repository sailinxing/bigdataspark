...
object IPLocation {
  ...
  val data2MySQL = (iterator: Iterator[(String, Int)]) => {
    ...
    try {
      conn = DriverManager.getConnection("jdbc:mysql://192.168.1.106:3306/spark", "root", "LIXINXIN123!")
  ...
  def main(args: Array[String]) {
    ...
    val ipRulesRdd = sc.textFile("D:/testsparkdata/ip/ip.txt").map(line => {
    ...
    //加载要处理的数据
    val ipsRDD = sc.textFile("D:/testsparkdata/ip/ip.txt").map(line => {
      val fields = line.split("\\|")
      fields(1)
    })
